...
1. **High-value transactions**: Transactions over $10,000 are flagged.
2. **Rapid multiple transactions**: More than 2 transactions by the same user within the same minute are flagged.
3. **Transactions from multiple countries**: Users with transactions from 3 or more different countries are flagged.
4. **Withdrawal bursts**: More than 3 withdrawal transactions by the same user within any rolling 10-minute window are flagged.
...
2. Alerts will be automatically generated based on the detection rules.
3. Query the `alerts` table joined with `users` and `locations` to view flagged transactions.
